...
describe("TestUniswapSwap", function () {
  ...
  before(async () => {
    ...
    testUniswapV2 = await ethers.getContract("TestUniswapSwapV2");
    tokenIn = await ethers.getContractAt("IERC20", TOKEN_IN);
    tokenOut = await ethers.getContractAt("IERC20", TOKEN_OUT);
    daiWhaleSigner = await impersonateAccount(DAI_WHALE);
  ...
  it("V2: Should swap", async function () {
    console.log(
